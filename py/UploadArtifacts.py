...
# Example usage:
# python3 UploadArtifacts.py all
...
params = os.path.join(os.getcwd(), 'parameters/aosm.names.parameters.json')
...
if sys.argv[1] == "all" or sys.argv[1] == "helm":
    ...
    with open(os.path.join(os.getcwd(), 'parameters/helm-charts.json'), 'r') as file:
    ...
    with open(os.path.join(os.getcwd(), 'parameters/images.json'), 'r') as file:
...
if sys.argv[1] == "all" or sys.argv[1] == "oras":
    ...
    # Build the ARM template from bicep to json
    command_build = ["az", "bicep", "build", "--file", "bicep/arm_cnf_template.bicep", "--outfile", "bicep/arm_cnf_template.json"]
    output = subprocess.check_output(command_build)

    # Push the ARM template to oras
    command_oras_push = ["oras", "push", acr_cred["acrServerUrl"].replace("https://", "") + "/nfdvtemplate:1.0.0", "bicep/arm_cnf_template.json"]
