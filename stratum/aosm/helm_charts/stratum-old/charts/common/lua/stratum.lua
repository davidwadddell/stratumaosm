...
function stratum.route_to_local_partition(txn)
  ...
  stratum_logging.debug_log(txn, c:dump())
...
function find_partition(txn)
  ...
  if local_partition_only ~= nil then
...
function handle_response(txn)
  local is_health_check = txn:get_var("res.health_check")

  -- Don't log health checks.
  if is_health_check == nil then
    local uuid = txn:get_var("res.uuid")
    if uuid == nil then
      uuid = ""
    end

    local client_ip = txn:get_var("res.client_ip")
    if client_ip == nil then
      client_ip = "Unknown IP"
    end

    local client_port = txn:get_var("res.client_port")
    if client_port == nil then
      client_port = "[Unknown Port]"
    end

    local method = txn:get_var("res.method")
    if method == nil then
    ...
    local start_time = txn:get_var("res.start_time")
    if start_time then
      start_time = tonumber(start_time)
      if start_time then
        local time_now = core.now()

        -- Convert to microseconds.
        local epoch_us = (time_now["sec"] * 1000000) + time_now["usec"]
        local rtt_us = epoch_us - start_time
  ...
  -- Let HAProxy know that we are done with this txn.
  core.done()
end
...
core.register_action("handle_response", {"http-res"}, handle_response, 0)
